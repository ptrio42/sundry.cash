...
JavaScript at all: `index.html` and `styles.css` are the whole page.

Written to `docs/landing-spec.md`. What follows is what the spec left to the implementation.
...
`performance.getEntriesByType('resource')` returns five entries and none of them leaves the origin.
Re-run that check after any edit — it is the one claim on the page that a single careless `<link>`
turns into a lie.
...
- **Newsreader has two strings here, not three** — the headline and the finding sentences. The
  wordmark is an SVG with outlined glyphs, so it carries its own type.
...
## Things on the page that are not true yet

- **`https://demo.sundry.cash` does not resolve** until the demo instance is deployed. The page
  links it in three places. Deploy before publishing, or the strongest asset on the page is a dead
  link.
- **"MIT licensed. The repository is not public yet — write and I will send it to you."** The moment
  the repo is public, that sentence in the *Self-host it* door and the footer's licence line should
  become a link, and the privacy section can go back to inviting people to read the code rather than
  only the network panel.
...
An empty array is the whole test. Read it at 375 px wide and in both colour schemes; the page has
no other states.
